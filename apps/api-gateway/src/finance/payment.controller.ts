...
export class PaymentController {
  ...
  @ApiOperation({
    summary: 'Fund escrow for a milestone',
    description:
      'Charges the client via Stripe and holds funds in escrow until the milestone is approved.',
  })
  @ApiResponse({
    status: 201,
    description: 'Escrow funded, PaymentIntent created',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Releases held funds to the freelancer via Stripe Connect payout.',
  })
  @ApiParam({
    name: 'milestoneId',
    description: 'UUID of the milestone to release',
    format: 'uuid',
  })
  @ApiResponse({
    status: 201,
    description: 'Escrow released, payout initiated',
  })
  @ApiResponse({ status: 429, description: 'Rate limit exceeded' })
  @Throttle({ strict: { limit: 10, ttl: 60_000 } })
