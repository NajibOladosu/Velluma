...
export class AutomationController {
  ...
  async createRule(@Body() data: CreateRuleDto) {
    return await firstValueFrom(this.client.send('create_rule', data));
  }

  @Get('rules/:tenantId')
  async listRules(@Param('tenantId') tenantId: string) {
    return await firstValueFrom(this.client.send('list_rules', { tenantId }));
  }

  @Post('trigger')
  async triggerEvent(@Body() data: TriggerEventDto) {
    return await firstValueFrom(this.client.send('trigger_event', data));
  }
}
