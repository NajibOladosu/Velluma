...
export class ExpenseController {
  ...
  @ApiOperation({
    ...
    description:
      "Moves the expense from pending → approved. The approver's identity is taken from the authenticated session.",
  })
  @ApiParam({ name: 'id', description: 'Expense UUID', format: 'uuid' })
  @ApiResponse({ status: 200, description: 'Approved expense record' })
  @ApiResponse({
    status: 404,
    description: 'Expense not found or not in pending state',
  })
  @Patch(':id/approve')
  ...
  @ApiOperation({
    ...
    description: 'Moves the expense from approved → reimbursed.',
  })
  @ApiParam({ name: 'id', description: 'Expense UUID', format: 'uuid' })
  @ApiResponse({ status: 200, description: 'Reimbursed expense record' })
  @Patch(':id/reimburse')
  async reimburseExpense(@Param('id', ParseUUIDPipe) id: string) {
    return callMicroservice(this.client.send('reimburse_expense', { id }));
  }
}
