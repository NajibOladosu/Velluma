...
export class ContractController {
  ...
  @ApiOperation({ summary: 'Generate an AI contract', description: 'Uses Google Gemini to generate a contract from a natural-language prompt. For Vercel deployments the Next.js route at /api/contracts/generate is used instead.' })
  ...
  @ApiOperation({ summary: 'Sign a contract', description: 'Records a digital signature. Once all required parties have signed the contract moves to pending_funding state.' })
  @ApiResponse({ status: 201, description: 'Signature recorded' })
  @ApiResponse({ status: 409, description: 'Contract already signed by this user' })
  ...
  @ApiParam({ name: 'projectId', description: 'Contract / project UUID', format: 'uuid' })
