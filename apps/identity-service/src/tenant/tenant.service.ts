...
export class TenantService {
    ...
    async getOrCreateTenant(ownerId: string, email: string) {
        ...
        try {
            const stripeAccount = await this.stripeService.createConnectAccount(email);
            await client
                .from('tenants')
    ...
    async getOnboardingLink(tenantId: string) {
        ...
        return await this.stripeService.createOnboardingLink(tenant.stripe_connect_id, tenant.slug);
    }
