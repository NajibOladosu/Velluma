...
export class TimeService {
    ...
    async startTimer(data: { projectId: string; userId: string; tenantId: string }) {
    ...
    async stopTimer(timerId: string) {
        ...
        const duration = (new Date().getTime() - new Date(timer.metadata.started_at).getTime()) / 1000;
        ...
        const { data: stopped, error } = await this.supabase
            ...
            .update({
                ...
                metadata: { ...timer.metadata, stopped_at: new Date().toISOString(), duration_seconds: duration },
            })
            .eq('id', timerId)
            .select()
            .single();
