...
export class PaymentService implements OnModuleInit {
    ...
    async releaseEscrow(milestoneId: string) {
        ...
        if (!transaction) throw new Error('No held transaction found for this milestone');
        if (!transaction.tenants?.stripe_connect_id) throw new Error('Freelancer has no Stripe account connected');
